<?php
/**
 * Title: Blog posts — introduction and cards
 * Slug: one-202x/show014_blog-posts
 * Categories: a2e
 * Description: An editable heading, introduction and two buttons above four latest blog posts, with images, categories and reading times. Use the Query Loop settings to adjust the posts and filters.
 * Keywords: blog, posts, articles, latest, news, cards
 * Viewport Width: 1440
 */

defined('ABSPATH') || exit;
?>
<!-- wp:group {"metadata":{"name":"Blog posts — introduction and cards"},"align":"full","className":"one-202x-pattern-show011_content-feed one-202x-pattern-show014_blog-posts","backgroundColor":"base","textColor":"contrast","layout":{"type":"default"}} -->
<div class="wp-block-group alignfull one-202x-pattern-show011_content-feed one-202x-pattern-show014_blog-posts has-contrast-color has-base-background-color has-text-color has-background">
    ...
    <div class="wp-block-group a2e-content-feed__intro">
        ...
        <div class="wp-block-group a2e-content-feed__description">
            ...
            <div class="wp-block-group a2e-button-group">
                <!-- wp:one-202x/icon-button {"backgroundColor":"accent","textColor":"base","className":"a2e-button","showIcon":false,"iconPosition":"right","text":<?php echo wp_json_encode(__('Primary Button', 'one-base-theme'), JSON_HEX_TAG | JSON_HEX_AMP); ?>} -->
                    <!-- wp:icon {"icon":"core/arrow-right","lock":{"move":true,"remove":true}} /-->
                <!-- /wp:one-202x/icon-button -->

                <!-- wp:one-202x/icon-button {"className":"is-style-outline a2e-button","backgroundColor":"base","textColor":"accent","borderColor":"accent","showIcon":false,"iconPosition":"right","text":<?php echo wp_json_encode(__('Secondary button', 'one-base-theme'), JSON_HEX_TAG | JSON_HEX_AMP); ?>} -->
                    <!-- wp:icon {"icon":"core/arrow-right","lock":{"move":true,"remove":true}} /-->
                <!-- /wp:one-202x/icon-button -->
            </div>
            <!-- /wp:group -->
    ...
    <div class="wp-block-query">
        ...
        <!-- wp:query-no-results -->
            <!-- wp:paragraph -->
            <p><?php esc_html_e('No blog posts found.', 'one-base-theme'); ?></p>
            <!-- /wp:paragraph -->
        <!-- /wp:query-no-results -->
    </div>
    <!-- /wp:query -->
