document.querySelectorAll('.one-202x-pattern-page008_faqs').forEach((page) => {
    ...
    // Search the complete rendered category lists, including collapsed answers.
    // Without JavaScript, search is hidden and every native accordion stays available.
    ...
    function filterAnswers() {
        ...
        status.textContent = words.length === 0 ? '' : empty
            ? status.dataset.emptyLabel
            : status.dataset.resultsLabel.replace('%d', String(count));
    }
