// wp-content/themes/one-base-theme/src/scripts/editor/query-selection.js
(function(wp) {
  "use strict";
  ...
  wp.blocks.registerBlockVariation("core/query", {
    ...
    allowedControls: [],
    scope: []
    // Insert the complete section through Patterns.
  });
  function Selection({ query, setAttributes }) {
    ...
    useEffect(() => {
      ...
      setType(null);
      setError(false);
      wp.apiFetch({ path: "/wp/v2/types/" + encodeURIComponent(query.postType), signal: controller.signal }).then(setType).catch(() => {
    ...
    function update(next) {
      const updated = {
        ...query,
        orderBy: next.length ? "include" : "date",
        order: "desc",
        perPage: next.length || query.one202xLatestCount || 3,
        offset: 0,
        inherit: false
      };
      delete updated.include;
      if (next.length) {
        updated.include = next;
      }
    ...
    return el(
      ...
      el(ComboboxControl, {
        ...
        onChange: (value) => {
          ...
          if (id > 0 && ids.length < 100 && !ids.includes(id)) {
            update([...ids, id]);
            setInput("");
          }
        },
      ...
      ids.length === 0 && el(RangeControl, {
        label: __("Number of recent items", "one-base-theme"),
        value: query.one202xLatestCount || 3,
        min: 1,
        max: 12,
  ...
  wp.hooks.addFilter("editor.BlockEdit", "one-202x/query-selection", (BlockEdit) => function QueryEdit(props) {
    ...
    return el(Fragment, null, el(BlockEdit, props), applies && props.isSelected && el(wp.blockEditor.InspectorControls, null, el(Selection, { query: props.attributes.query, setAttributes: props.setAttributes })));
  });
})(window.wp);
